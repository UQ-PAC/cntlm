...
if [[ "$1" == build ]]; then
  ...
  set -x
	exec $DOCKER run -v$GIT_ROOT:/$GIT_ROOT --rm -td --user root --name $DOCKER_CONTAINER $DOCKER_IMAGE
elif [[ "$1" == stop ]]; then
  set -x
...
set -x
exec $DOCKER exec --user root -it -w "$(pwd)" $DOCKER_CONTAINER /usr/bin/_exec "$@"
